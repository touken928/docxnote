...
def test_sources_and_literal_pipelines():
    ...
    assert [r['path'] for r in records(shell.run("docx | grep -F '付款' | grep -v '示例'"))] == ['p:0']
...
@pytest.mark.parametrize('command', [
    '', 'wc -l', 'sed -n 1p', 'cat file', 'echo hi', 'grep hi',
    'docx |', '| docx', 'docx || head', 'docx ; docx', 'docx > file',
    'docx && docx', 'docx\ndocx', 'docx | grep --unknown hi',
    'docx | head -n -1', 'docx | tail -n x', 'docx | head file',
    'docx | grep', "docx | grep 'broken", 'docx p:999', 'docx p:0#0',
    'docx | comment p:0 hi', 'comment p:0 hi | head',
    'comment p:0 hi --start 0', 'comment p:0 hi --quote ""',
    'docx p:0 --start -1', 'docx p:0 --start 3 --end 2',
])
...
def test_comment_quote_validation_and_roundtrip():
    ...
    assert (added['target'], added['start'], added['end'], added['author']) == ('p:0', 3, 5, 'reviewer')
...
def test_overlapping_quote_matches_are_ambiguous():
    assert DocxShell(make_doc('aaa')).run("comment p:0 note --quote aa")['exit_code'] == 2
...
def test_limit_is_after_filtering_and_json_remains_valid():
    ...
    next_part = records(shell.run(f"docx p:0 --start {preview['end']} --end {preview['end'] + 20}"))[0]
